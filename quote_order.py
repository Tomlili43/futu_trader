...
# 开仓函数
def open_position(code,last_price):
    # 获取摆盘数据
    # ask, bid = get_ask_and_bid(code)

    # 计算下单量
    open_quantity = calculate_quantity(last_price)

    # 判断购买力是否足够
    # if is_valid_quantity(TRADING_SECURITY, open_quantity, 80):
    # 下单
    ret, data = trade_context.place_order(price=last_price, qty=open_quantity, code=code, trd_side=TrdSide.BUY,
...
# 计算下单数量
def calculate_quantity(last_price):
    price_quantity = 0
    # 使用最小交易量
    ...
    # maximum_quantity
    ret, data = quote_context.get_market_snapshot([TRADING_SECURITY])
    ret, data = trade_context.acctradinginfo_query(order_type=OrderType.NORMAL, code=TRADING_SECURITY, price=last_price,trd_env=TRADING_ENVIRONMENT)
    if ret != RET_OK:
        print('获取快照失败：', data)
        return price_quantity
    price_quantity = data['max_cash_buy'][0]
    print('【下单数量】', price_quantity)
    return price_quantity


# 判断购买力是否足够 price should  be last_price
...
class StockQuoteClass(StockQuoteHandlerBase):
    def on_recv_rsp(self, rsp_pb):
        ret_code, data = super(StockQuoteClass,self).on_recv_rsp(rsp_pb)
        if ret_code != RET_OK:
            print("StockQuoteClass: error, msg: %s"% data)
            return RET_ERROR, data
        #  print("StockQuoteClass ", data) # StockQuoteClass's own processing logic
        last_price = data['last_price'].values[0]
        if(last_price < 89.0):
            print("last_price < 89")
            open_position(TRADING_SECURITY,last_price)
        return RET_OK, data
...
if __name__ == '__main__':
    ...
    if not on_init():
        ...
        quote_context.set_handler(StockQuoteClass())
        ...
        quote_context.subscribe(code_list=[TRADING_SECURITY], subtype_list=[SubType.QUOTE,SubType.TICKER, SubType.ORDER_BOOK, TRADING_PERIOD])
